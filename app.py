...
class LeetAnalysis(webdriver.Chrome):
    ...
    def __init__(self, driver_path="/usr/local/bin/chromedriver", teardown=False):
        ...
        super(LeetAnalysis, self).__init__(service=service, options=options)
...
def main():
    ...
    if not df.empty:
        st.write("LeetCode Submissions Data")
        st.dataframe(df)
    else:
    ...
    if st.button('Run Scraper'):
        with st.spinner('Scraping in progress...'):
            scraper = LeetAnalysis(teardown=True)
            scraper.load_df()
            scraper.land_first_page()
            scraper.login()
            scraper.scrap_submission_page()
            scraper.scrap_each_page()
